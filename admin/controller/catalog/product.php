...
class ControllerCatalogProduct extends Controller {
	...
	public function filter() {
		...
		foreach ($results as $result) {
			...
			if ($result['discount_image'] && file_exists(DIR_IMAGE . $result['discount_image'])) {
				$discount_image = $this->model_tool_image->resize($result['discount_image'], 40, 40);
			} else {
				$discount_image = $this->model_tool_image->resize('no_image.jpg', 40, 40);
			}
  	...
  	protected function getForm() {
		...
		$this->data['entry_type_of_discount'] = $this->language->get('entry_type_of_discount');
		$this->data['discount_currency'] = $this->language->get('discount_currency');
		...
		if (isset($this->request->post['discount_image'])) {
			$this->data['discount_image'] = $this->request->post['discount_image'];
		} elseif (!empty($product_info)) {
			$this->data['discount_image'] = $product_info['discount_image'];
		} else {
			$this->data['discount_image'] = '';
		}
		...
		if (isset($this->request->post['discount_image']) && file_exists(DIR_IMAGE . $this->request->post['discount_image'])) {
			$this->data['discount_thumb'] = $this->model_tool_image->resize($this->request->post['discount_image'], 40, 40);
		} elseif (!empty($product_info) && $product_info['discount_image'] && file_exists(DIR_IMAGE . $product_info['discount_image'])) {
			$this->data['discount_thumb'] = $this->model_tool_image->resize($product_info['discount_image'], 40, 40);
		} else {
			$this->data['discount_thumb'] = $this->model_tool_image->resize('no_image.jpg', 40, 40);
			$this->data['discount_image'] = 'no_image.jpg';
		}
		...
		if (isset($this->request->post['my_discount'])) {
      		$this->data['my_discount'] = $this->request->post['my_discount'];
    	} elseif (!empty($product_info)) {
			$this->data['my_discount'] = $product_info['my_discount'];
		} else {
      		$this->data['my_discount'] = '';
    	}
		
		if (isset($this->request->post['discount_type'])) {
			$this->data['discount_type'] = $this->request->post['discount_type'];
		} elseif (!empty($product_info)) {
		$this->data['discount_type'] = $product_info['discount_type'];
		} elseif ($this->request->post[1]) {
			$this->data['discount_type'] = 1;
		} else {
			$this->data['discount_type'] = 2;
		}




		if (isset($this->request->post['discount_image_width'])) {
			$this->data['discount_image_width'] = $this->request->post['discount_image_width'];
		} elseif (!empty($product_info)) {
			$this->data['discount_image_width'] = $product_info['discount_image_width'];
		} else {
			$this->data['discount_image_width'] = '';
		}

		if (isset($this->request->post['discount_image_height'])) {
			$this->data['discount_image_height'] = $this->request->post['discount_image_height'];
		} elseif (!empty($product_info)) {
			$this->data['discount_image_height'] = $product_info['discount_image_height'];
		} else {
			$this->data['discount_image_height'] = '';
		}
		...
		if (isset($this->request->post['discount_image'])) {
			$discount_images = $this->request->post['discount_image'];
		} elseif (isset($this->request->get['product_id'])) {
			$discount_images = $this->model_catalog_product->getDiscountImages($this->request->get['product_id']);
		} else {
			$discount_images = array();
		}

		$this->data['discount_images'] = array();

		foreach ($discount_images as $discount_image) {
			if ($discount_image['discount_image'] && file_exists(DIR_IMAGE . $discount_image['discount_image'])) {
				$discount_image = $discount_image['discount_image'];
			} else {
				$discount_image = 'no_image.jpg';
			}

			$this->data['$discount_image'][] = array(
				'discount_image'      => $discount_image,
				'discount_thumb' => $this->model_tool_image->resize($discount_image, 40, 40),
